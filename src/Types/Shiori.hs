...
-- | Installation result
data InstallResult = InstallSuccess | InstallFailure | InstallRefuse
  deriving ( Show, Eq )

-- | Network update result
data UpdateResult = UpdateSuccess | UpdateFailure | UpdateNone       -- ^ No updates available
  deriving ( Show, Eq )
...
parseGhostStatus t = case T.strip t of
  ...
  other
    | "opening(" `T.isPrefixOf` other && ")" `T.isSuffixOf` other -> let
        content = T.dropEnd 1 $ T.drop 8 other  -- drop "opening(" and ")"
        types   = T.splitOn "/" content
      in 
        Just $ StatusOpening types
    | "balloon(" `T.isPrefixOf` other && ")" `T.isSuffixOf` other -> let
        content     = T.dropEnd 1 $ T.drop 8 other  -- drop "balloon(" and ")"
        ...
        parsePair p = case T.splitOn "=" p of
          ...
          _ -> Nothing
      in 
        Just $ StatusBalloon (mapMaybe parsePair pairs)
    | otherwise -> Nothing
...
serializeGhostStatus (StatusBalloon pairs)
  = "balloon("
  <> T.intercalate "/" (map (\( c, b ) -> T.pack (show c) <> "=" <> T.pack (show b)) pairs)
  <> ")"
...
data BalloonOffset = BalloonOffset { boX :: !Int, boY :: !Int }
  deriving ( Show, Eq )
...
parseShioriRequest input = do
  ...
  where
        ...
        in 
          case parts of
            [ methodStr, versionStr ] -> do
              method <- maybe
                (Left $ "Unknown method: " <> methodStr)
                Right
                (parseShioriMethod methodStr)
              version <- maybe
                (Left $ "Unknown version: " <> versionStr)
                Right
    ...
    parseHeader acc line = case T.breakOn ": " line of
      ( key, rest )
    ...
    parseReferences = Map.foldlWithKey' go Map.empty
      where
        go acc key val
          | "Reference" `T.isPrefixOf` key = case T.stripPrefix "Reference" key of
            Just numStr -> case reads (T.unpack numStr) of
    ...
    parsePassThruHeaders = Map.foldlWithKey' go Map.empty
      where
        go acc key val
          | "X-SSTP-PassThru-" `T.isPrefixOf` key = case T.stripPrefix "X-SSTP-PassThru-" key of
