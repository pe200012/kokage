...
compositeWithCairo :: Pixbuf.Pixbuf -> [ ( Pixbuf.Pixbuf, Int, Int, DrawMethod ) ] -> IO (Maybe Pixbuf.Pixbuf)
compositeWithCairo first pixbufs = do
  ...
  Cairo.withImageSurface Cairo.FormatARGB32 (fromIntegral width) (fromIntegral height) $ \surface -> do
