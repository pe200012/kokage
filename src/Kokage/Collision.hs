...
hitTestShape x y shape = case shape of
  ...
  CollisionEllipse x1 y1 x2 y2 ->
    ...
    let
        cx = fromIntegral (x1 + x2) / 2.0 :: Double
      ...
      in 
        rx > 0 && ry > 0 && (dx * dx + dy * dy) <= 1.0

  CollisionCircle cx cy r      -> let
      dx = x - cx
      dy = y - cy
    in 
      dx * dx + dy * dy <= r * r

  CollisionPolygon vertices    -> pointInPolygon x y vertices
...
pointInPolygon px py vertices = odd $ length $ filter crossesRay edges
  where
    ...
    crossesRay ( ( x1, y1 ), ( x2, y2 ) )
      = let
          -- Check if the horizontal ray from (px, py) to the right crosses this edge
          minY       = min y1 y2
          maxY       = max y1 y2
          ...
          xIntersect
            = fromIntegral x1
              + (fromIntegral (py - y1) * fromIntegral (x2 - x1)) / fromIntegral (y2 - y1) :: Double
        in 
          inYRange && fromIntegral px < xIntersect
