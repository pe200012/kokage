...
initBalloonPosition cs shell = do
  ...
  (finalDir, finalX) <- case dir of
    BalloonRight ->
      if rightPosX + balloonW > monX + monW && leftPosX >= monX
        then do
          -- Right side overflows, left side fits -> flip to left
          putStrLn $ "[Character " <> show (csScopeId cs) <> "] Flipping balloon to left (edge detection)"
          return (BalloonLeft, leftPosX)
        else
          -- Keep right, clamp if needed
          return (BalloonRight, max monX (min rightPosX (monX + monW - balloonW)))
    BalloonLeft ->
      if leftPosX < monX && rightPosX + balloonW <= monX + monW
        then do
          -- Left side overflows, right side fits -> flip to right
          putStrLn $ "[Character " <> show (csScopeId cs) <> "] Flipping balloon to right (edge detection)"
  ...
  when (finalDir /= dir) $ do
    ...
    let charType = if csScopeId cs == 0 then "s" else "k"
  ...
  putStrLn $ "[Character " <> show (csScopeId cs) <> "] Balloon position:"
          <> " char=(" <> show charX <> "," <> show charY <> ")"
          <> " surfSize=(" <> show surfW <> "," <> show surfH <> ")"
          <> " balloonSize=" <> show balloonSize
          <> " dir=" <> show finalDir
          <> " monitor=(" <> show monX <> "," <> show monY <> "," <> show monW <> "," <> show monH <> ")"
          <> " -> balloon=(" <> show finalX <> "," <> show finalY <> ")"
...
getMonitorGeometry pointX pointY = do
  ...
  case mDisplay of
    ...
    Just display -> do
      ...
      putStrLn $ "[MonitorDebug] Looking for point (" <> show pointX <> "," <> show pointY <> ") in " <> show n <> " monitors"
      allMonitors <- collectAllMonitors monitors n 0 []
      mapM_ (\(i, mx, my, mw, mh) ->
        putStrLn $ "[MonitorDebug]   Monitor " <> show i <> ": (" <> show mx <> "," <> show my <> ") " <> show mw <> "x" <> show mh) allMonitors
      findMonitorContainingPoint monitors n 0
  where
    collectAllMonitors :: Gio.ListModel -> Word32 -> Word32 -> [(Int, Int, Int, Int, Int)] -> IO [(Int, Int, Int, Int, Int)]
      ...
      | otherwise = do
          ...
          case mObj of
            ...
            Just obj -> do
              ...
              collectAllMonitors monitors total (idx + 1) ((fromIntegral idx, fromIntegral mx, fromIntegral my, fromIntegral mw, fromIntegral mh) : acc)
      ...
      | otherwise = do
          ...
          case mObj of
            ...
            Just obj -> do
              ...
              if pointX >= monX && pointX < monX + monW &&
                 pointY >= monY && pointY < monY + monH
...
updateBalloonPosition cs dx dy = do
  ...
  Balloon.updateBalloonPositionWithChar
    (csBalloon cs)
    dx
    dy
    (fromIntegral charX)
