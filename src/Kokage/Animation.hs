...
tickAnimations animState _shell surfDef activeAnims currentTimers delta = do
  ...
  (finalAnims, finalTimers, newStarted) <- foldM (checkInterval delta) (animsAfterQueue, currentTimers, False) candidates

  -- 6. Add enabled bind animations
  enabledBinds <- readIORef (asBindState animState)
  let bindCandidates = filter (\a -> isBindInterval (animInterval a) 
  ...
  return (animsWithBinds, finalTimers, changed || newStarted || cmdStarted || queueStarted || bindStarted)
...
executeCommands surfDef anims cmds = foldM execCmd anims cmds
  where
    execCmd acc cmd = case cmd of
      ...
      CmdStop aid ->
        return $ filter (\a -> animId (aaDef a) /= aid) acc
      
      CmdStartAll aids -> do
        let runningIds = map (animId . aaDef) acc
            toStart = filter (`notElem` runningIds) aids
        newAnims <- catMaybes <$> mapM (\aid -> 
          case findAnimById aid surfDef of
            Nothing -> return Nothing
            Just anim -> startAnimation anim False) toStart
        return (acc ++ newAnims)
      
      CmdStopAll aids ->
        return $ filter (\a -> animId (aaDef a) `notElem` aids) acc
    
    findAnimById :: Int -> SurfaceDefinition -> Maybe Animation
    findAnimById aid sd = 
      case filter (\a -> animId a == aid) (sdAnimations sd) of
        [] -> Nothing
        (x:_) -> Just x
...
invokeAnimation surfDef activeAnims animationId force = do
  ...
  case mAnim of
    [] -> return activeAnims
    (anim : _) ->
      if animationId `elem` runningIds && not force
        then return activeAnims
        else do
          -- Remove existing if force
          let filtered = if force
...
stopAnimation activeAnims animationId =
  filter (\a -> animId (aaDef a) /= animationId) activeAnims
...
toggleBind animState animId = do
  modifyIORef' (asBindState animState) $ \binds ->
    if Set.member animId binds
