...
import           Kokage.Config              ( GhostHistory(..), loadGhostHistory
                                            , saveGhostHistory, defaultGhostHistory )
import           Kokage.Shiori.WineBridge   ( WineShiori, sendRequest )
import           Types.Ghost                ( Ghost(..), ghostDescript, descriptSakuraName )
import           Types.Shiori               ( ShioriResponse(..), mkRequest, eventToId
                                            , ShioriEvent(..) )
...
initializeShiori shiori ghost = do
  ...
  let refs = Map.fromList
        ...
        , (1, T.pack $ show $ if isFirst then 0 else ghBootCount hist)  -- Boot count
  ...
  let event = if isFirst then OnFirstBoot else OnBoot
      eventId = eventToId event
      req = mkRequest "Kokage" eventId refs
  
  result <- try (sendRequest shiori req) :: IO (Either SomeException (Either String ShioriResponse))
  case result of
    Left err -> putStrLn $ "[SHIORI] Boot error: " <> show err
    Right (Left err) -> putStrLn $ "[SHIORI] Boot error: " <> err
    Right (Right resp) -> 
      let val = fromMaybe "" (srsValue resp)
      in putStrLn $ "[SHIORI] Boot response: " <> T.unpack val
  
  -- Update history
  let newHist = hist
        { ghBootCount = ghBootCount hist + 1
...
cleanupShiori shiori gPath startTime = do
  ...
  case result of
    ...
    Right (Right resp) -> 
      let val = fromMaybe "" (srsValue resp)
      in putStrLn $ "[SHIORI] Close response: " <> T.unpack val
